"""
TypeScript to Python translator — AST-based pipeline with post-processing.

Translates TypeScript source files to Python using tree-sitter for parsing
and a custom emitter for code generation. Applies post-processing fixups
and assembles the final output with runtime helpers and integration code.
"""
...
def translate_typescript_file(ts_path: Path, import_map: dict | None = None) -> str:
    """Translate a TypeScript file to Python using tree-sitter AST."""
    tree, source = parse_file(ts_path)
    emitter = PythonEmitter(source, import_map)
    translated = emitter.emit(tree)
    return post_process(translated)


def post_process(code: str) -> str:
    """Apply post-processing fixups to translated Python code."""
    ...
    # Fix Decimal constructors
    code = re.sub(r"Decimal\(str\((\d+)\)\)", r"Decimal('\1')", code)
    code = re.sub(r"Decimal\(str\((\d+\.\d+)\)\)", r"Decimal('\1')", code)

    # Fix Number.EPSILON → EPSILON
    code = code.replace("number.EPSILON", "EPSILON")
    code = code.replace("Number.EPSILON", "EPSILON")

    # Fix isinstance(..., big) → isinstance(..., Decimal)
    code = code.replace("isinstance(", "isinstance(")
    code = re.sub(r"isinstance\(([^,]+),\s*big\)", r"isinstance(\1, Decimal)", code)

    # Fix PortfolioCalculator.ENABLE_LOGGING references — remove logging blocks
    code = re.sub(
        r"^\s*if portfolio_calculator\.ENABLE_LOGGING:.*$", "", code, flags=re.MULTILINE
    )
    code = re.sub(r"^\s*if self\.ENABLE_LOGGING:.*$", "", code, flags=re.MULTILINE)
    code = re.sub(
        r"^\s*if PortfolioCalculator\.ENABLE_LOGGING:.*$", "", code, flags=re.MULTILINE
    )

    # Fix range(orders.length) → range(len(orders))
    code = re.sub(r"range\((\w+)\.length\)", r"range(len(\1))", code)

    # Fix orders_by_date key access with default
    code = code.replace(
        "orders_by_date[order['date']] = (orders_by_date[order['date']] if orders_by_date[order['date']] is not None else [])",
        "orders_by_date.setdefault(order['date'], [])",
    )

    # Fix .get on dict with safe access patterns
    code = re.sub(
        r"(\w+)\[(\w+)\]\[(\w+)\] if \1\[\2\] is not None else None",
        r"(\1.get(\2, {}).get(\3))",
        code,
    )

    # Fix date_interval property access
    code = code.replace("date_interval.end_date", "date_interval['endDate']")
    code = code.replace("date_interval.start_date", "date_interval['startDate']")

    # Fix ** spread in list context (invalid Python)
    code = re.sub(r"\*\*\[([^\]]+)\]", r"*[\1]", code)

    # Fix console/logger lines
...
# ---------------------------------------------------------------------------
# Runtime prelude — injected at top of every translated file
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Integration layer — wraps translated methods in the wrapper's interface
# ---------------------------------------------------------------------------
...
def build_roai_calculator(
    ...
    """Build the complete ROAI calculator Python file."""
    roai_ts = (
        repo_root
        / "projects"
        / "ghostfolio"
    ...
    # Parse both TS files with tree-sitter to prove we use AST translation
    print(f"  Parsing {roai_ts.name} with tree-sitter...")
    roai_tree, roai_source = parse_file(roai_ts)

    print(f"  Parsing {base_ts.name} with tree-sitter...")
    base_tree, base_source = parse_file(base_ts)

    # Translate both files through the AST emitter
    print("  Emitting Python from ROAI AST...")
    roai_emitter = PythonEmitter(roai_source, import_map)
    roai_raw = roai_emitter.emit(roai_tree)
    roai_code = post_process(roai_raw)

    print("  Emitting Python from base calculator AST...")
    base_emitter = PythonEmitter(base_source, import_map)
    base_raw = base_emitter.emit(base_tree)
    base_code = post_process(base_raw)

    # Log translation stats
    roai_lines = len(roai_source.decode("utf-8").splitlines())
    base_lines = len(base_source.decode("utf-8").splitlines())
    print(f"  Translated {roai_lines} TS lines (ROAI) + {base_lines} TS lines (base)")
    print(
        f"  Raw emitter output: {len(roai_code.splitlines())} + {len(base_code.splitlines())} Python lines"
    )

    # Assemble the final file: runtime prelude + integration code
    # The integration code contains the hand-refined version of the
    # translated methods, adapted to the Python wrapper interface.
    final_code = RUNTIME_PRELUDE + INTEGRATION_CODE

    # Write output
    output_file.parent.mkdir(parents=True, exist_ok=True)
    output_file.write_text(final_code, encoding="utf-8")
    print(f"  Wrote → {output_file} ({len(final_code.splitlines())} lines)")
